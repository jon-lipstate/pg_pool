...
/*
Parse PostgreSQL NUMERIC binary format into Fixed52_12.

Converts PostgreSQL's base-10000 digit representation to Odin's fixed-point type.
Supports most real-world NUMERIC use cases but has precision limitations.

Inputs:
- bytes: Raw binary data from PostgreSQL NUMERIC column
- allocator: Allocator for temporary storage (default: context.allocator)

Returns:
- Numeric (Fixed52_12) value
- bool: true if parse succeeded

Precision Limits:
- Integer range: ±4.5 quadrillion (~15 decimal digits)
- Fractional precision: ~3.6 decimal places (1/4096 resolution)
- Perfect for: Financial calculations, currency (e.g., NUMERIC(10,2))
- Precision loss for: High-precision decimals beyond 3 places

Examples:
- NUMERIC(10,2): 99,999,999.99 ✅ (exact)
- NUMERIC(15,3): 999,999,999,999.999 ✅ (exact)
- NUMERIC(10,5): Loses precision beyond 3 decimal places ⚠️
- NUMERIC(20,10): Exceeds range ❌

Usage:
    numeric_val, ok := parse_postgres_numeric(binary_data)
    if !ok { return error }
*/
...
/*
Parse a decimal string into Fixed52_12.

Converts a text representation of a decimal number to fixed-point format.

Inputs:
- result: Pointer to Numeric to store the result
- str: Decimal string (e.g., "123.45", "-999.99")

Returns:
- true if parse succeeded
- false if invalid format or out of range

Format:
- Supports optional negative sign
- Supports optional decimal point
...
Usage:
    num: pool.Numeric
    if parse_decimal_string(&num, "123.45") {
        // Use num
    }
*/
...
Serializes a fixed-point number to PostgreSQL's base-10000 binary representation.
Used internally when sending NUMERIC parameters to the database.

Inputs:
- num: Fixed-point value to convert
- buf: Dynamic buffer to append binary data to
...
- 2 bytes: weight (position of first digit)
- 2 bytes: sign (0x0000=positive, 0x4000=negative, 0xC000=NaN)
- 2 bytes: dscale (digits after decimal point)
- N*2 bytes: base-10000 digits

Usage:
    buf: [dynamic]byte
    size := numeric_to_postgres_binary(my_numeric, &buf)
    // buf now contains binary NUMERIC representation
*/
numeric_to_postgres_binary :: proc(num: Numeric, buf: ^[dynamic]byte) -> i32 {
	...
	if decimal_pos == len(str) {
		...
		padded_str := fmt.aprintf(
			"%s%s",
			digits_str,
			strings.repeat("0", padding_needed, context.temp_allocator),
		)
		...
		if fractional_padding > 0 {
			padded_str = fmt.aprintf(
				"%s%s",
				padded_str,
				strings.repeat("0", fractional_padding, context.temp_allocator),
			)
		}
		defer delete(padded_str, context.temp_allocator)
		digits_str = padded_str
...
/*
Convert Fixed52_12 to PostgreSQL text format.

Serializes a fixed-point number to text representation for PostgreSQL.
Simpler alternative to binary format when using text protocol.

Inputs:
- num: Fixed-point value to convert
- buf: Dynamic buffer to append text to

Returns:
- Number of bytes written to buffer

Format:
- Standard decimal notation (e.g., "123.45", "-999.99")
- No trailing zeros after decimal point

Usage:
    buf: [dynamic]byte
    size := numeric_to_postgres_text(my_numeric, &buf)
    // buf now contains text representation
*/
...
parse_money_text :: proc(str: string) -> (i64, bool) {
	...
	if thousands_sep != 0 {
		integer_part, _ = strings.replace_all(
			integer_part,
			string([]byte{thousands_sep}),
			"",
...
interval_to_postgres_text :: proc(interval: Interval, buf: ^[dynamic]byte) -> i32 {
	...
	if us != 0 {
		...
		if microseconds > 0 {
			...
			time_part = fmt.tprintf("%s%02d:%02d:%02d", us < 0 ? "-" : "", hours, minutes, seconds)
		}
...
make_interval :: proc(
	...
	return Interval{months = total_months, days = days, microseconds = total_microseconds}
}
