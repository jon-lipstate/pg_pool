...
test_basic_queries :: proc() {
	...
	rows, query_err := pool.query(
		"SELECT id, email, name, age FROM users WHERE email = $1",
...
test_parameterized_queries :: proc() {
	...
	pool.exec(
		"INSERT INTO products (name, price, stock) VALUES ($1, $2, $3)",
		args = {"Mouse", 29.99, 100},
	)
	pool.exec(
		"INSERT INTO products (name, price, stock) VALUES ($1, $2, $3)",
		args = {"Keyboard", 79.99, 50},
	)
...
test_null_handling :: proc() {
	...
	pool.exec(
		"INSERT INTO products (name, price, stock, description) VALUES ($1, $2, $3, $4)",
		args = {"NullProduct", 10.00, 5, nil},
	)

	// Query users and handle NULL age
	rows, err := pool.query(
		"SELECT name, age FROM users WHERE email IN ($1, $2) ORDER BY name",
	...
	prod_rows, prod_err := pool.query(
		"SELECT description FROM products WHERE name = $1",
		args = {"NullProduct"},
	)
	if prod_err != nil {
