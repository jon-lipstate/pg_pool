...
test_dates_and_times :: proc() {
	...
	test_data := []struct {
		...
		{"2025-06-15", "2025-06-15 12:30:45.123456", "2025-06-15 12:30:45.123456+00", "Year 2025 mid-year"},
		{"1999-12-31", "2000-01-01 00:00:00", "2000-01-01 00:00:00+00", "Year 1999 (before epoch)"},
		{"2024-02-29", "2024-02-29 15:45:30", "2024-02-29 15:45:30+00", "Leap year date"},
	}
	
	// Insert test data
	for data in test_data {
		pool.exec("INSERT INTO test_dates (test_date, test_timestamp, test_timestamptz, description) VALUES ($1, $2, $3, $4)",
	...
	rows, err := pool.query("SELECT test_date, test_timestamp, description FROM test_dates ORDER BY id")
	if err != nil {
...
test_simple_tz :: proc() {
	...
	// Insert test data
	pool.exec("INSERT INTO test_tz (ts, tstz) VALUES ($1, $2)", args = {"2025-01-15 12:00:00", "2025-01-15 12:00:00+00"})
