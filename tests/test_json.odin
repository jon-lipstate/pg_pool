...
test_json :: proc() {
	...
	metadata := json.Object{
		...
		"tags" = json.Array{
			json.String("important"),
			json.String("reviewed"),
		},
	...
	settings := json.Object{
		...
		"limits" = json.Object{
			"max_size" = json.Float(1024),
			"timeout" = json.Float(30),
		},
	}
	...
	rows, query_err := pool.query("SELECT id, metadata, settings, description FROM test_json ORDER BY id")
	if query_err != nil {
	...
	contains_rows, _ := pool.query("SELECT id FROM test_json WHERE settings @> $1::jsonb", args = {`{"theme": "dark"}`})
