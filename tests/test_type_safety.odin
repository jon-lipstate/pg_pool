...
test_type_safety :: proc() {
	...
	pool.exec("CREATE TABLE test_type_safety (int2s INT2[], int4s INT4[], int8s INT8[], floats FLOAT4[], doubles FLOAT8[])")
	
	// Insert test data
	pool.exec("INSERT INTO test_type_safety VALUES (ARRAY[1,2,3]::INT2[], ARRAY[100,200,300]::INT4[], ARRAY[1000,2000,3000]::INT8[], ARRAY[1.5,2.5,3.5]::FLOAT4[], ARRAY[10.1,20.2,30.3]::FLOAT8[])")
	...
	if pool.next_row(&rows4) {
		...
		if err != pool.QueryError.None {
			...
			fmt.printf("INT8[] as []i32: ERROR - should have failed but got: %v ✗\n", int8_as_i32)
		}
	}
	...
	pool.exec("CREATE TABLE test_numeric (id SERIAL, price NUMERIC(10,2), amount DECIMAL(15,4), total NUMERIC(20,6))")
	...
	pool.exec("INSERT INTO test_numeric (price, amount, total) VALUES ($1, $2, $3)", args = {price, amount, total})
	
	// Also test f64 to NUMERIC conversion  
	pool.exec("INSERT INTO test_numeric (price, amount, total) VALUES ($1, $2, $3)", args = {99.99, 1234.5678, 5555.555555})
	...
	for pool.next_row(&rows7) {
		...
		fmt.printf("Price: %.2f, Amount: %.4f, Total: %.6f ✓\n", price_f64, amount_f64, total_f64)
	}
