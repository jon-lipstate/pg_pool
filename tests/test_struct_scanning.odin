...
test_struct_scanning :: proc() {
	...
	rows, err := pool.query("SELECT id, email, name, age, is_active, created_at FROM users WHERE email = $1", args = {"alice@example.com"})
	if err != nil {
	...
	if pool.next_row(&rows) {
		...
		fmt.printf("Struct scan: User{id=%d, email=%s, full_name=%s, age=%d, active=%t, created=%04d-%02d-%02d %02d:%02d:%02d}\n", 
			user.id, user.email, user.full_name, user.age, user.active,
	...
	prod_rows, prod_err := pool.query("SELECT id, name, price, stock, description FROM products ORDER BY id")
	if prod_err != nil {
	...
	for pool.next_row(&prod_rows) {
		...
		fmt.printf("  Product{id=%d, name=%s, price=%.2f, stock=%d, desc=%s}\n", 
			product.product_id, product.product_name, product.price, product.stock_count, product.desc)
