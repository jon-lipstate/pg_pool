...
test_binary_format :: proc() {
	...
	fmt.printf("Created test date: %04d-%02d-%02d (expected 2025-01-15)\n", time.year(test_date), time.month(test_date), time.day(test_date))
	ts_year, ts_month, ts_day := time.date(test_timestamp)
	ts_hour, ts_min, ts_sec := time.clock(test_timestamp)
	fmt.printf("Created test timestamp: %04d-%02d-%02d %02d:%02d:%02d (expected 2025-01-15 14:30:45)\n", 
		ts_year, int(ts_month), ts_day, ts_hour, ts_min, ts_sec)
	...
	`, args = {bool_val, int2_val, int4_val, int8_val, float4_val, float8_val, text_val, bytea_val, test_date, test_timestamp, test_json})
	...
	text_rows, text_err := pool.query("SELECT bool_val, int2_val, text_val, bytea_val, json_val FROM test_binary", result_format = .Text)
	if text_err != nil {
	...
	if pool.next_row(&text_rows) {
		...
		fmt.printf("  Text format: bool=%s, int2=%s, text='%s', bytea=%s, json='%s'\n", 
			bool_str, int2_str, text_str, bytea_str, json_str)
	}
	...
	if pool.next_row(&rows) {
		...
		fmt.printf("  timestamp: %04d-%02d-%02d %02d:%02d:%02d.%06d\n",
		           ts_year, int(ts_month), ts_day, ts_hour, ts_min, ts_sec, ts_nano/1000)
...
test_custom_types :: proc() {
	...
	// Insert UUID
	_, insert_err := pool.exec("INSERT INTO test_custom (uuid_val, test_name) VALUES ($1, $2)", args = {test_uuid, "Test UUID"})
	if insert_err != nil {
	...
	// Query UUID back
	rows, query_err := pool.query("SELECT uuid_val FROM test_custom WHERE test_name = $1", args = {"Test UUID"})
	if query_err != nil {
