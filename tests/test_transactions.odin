...
test_transactions :: proc() {
	...
	stock_rows, stock_err := pool.query(
	...
	_, update_err := pool.exec(
		"UPDATE products SET stock = $1 WHERE name = $2",
		tx,
		args = {new_stock, "Laptop"},
	)
	if update_err != nil {
	...
	verify_rows, verify_err := pool.query(
		"SELECT stock FROM products WHERE name = $1",
		tx,
		args = {"Laptop"},
	)
...
test_nested_transactions :: proc() {
	...
	rows, query_err := pool.query(
		"SELECT name FROM users WHERE email IN ($1, $2) ORDER BY name",
